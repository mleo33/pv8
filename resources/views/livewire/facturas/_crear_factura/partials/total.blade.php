<div class="card">
  <div class="card-header">
    ...
    <h3 class="card-title font-weight-bold"><i class="fas fa-dollar-sign"></i> Total a Facturar</h3>
    <div class="card-tools">
      {{-- <button class="btn btn-xs btn-danger" wire:click="limpiarCotizacion()"><i class="fas fa-broom"></i> Limpiar Renta</button> --}}
    </div>
  </div>
    ...
    <div>
      <center>
        @if (!$this->factura_t->model_id)
          <button data-toggle="modal" data-target="#mdlSelectVenta" class="btn btn-sm btn-primary"><i class="fas fa-shopping-basket"></i> Seleccionar Venta</button>
        @elseif ($this->factura_t->model_type == 'App\\Models\\Venta')
          <a href="/venta/{{$this->factura_t->model_id}}" target="_blank" class="btn btn-md btn-primary"><i class="fas fa-shopping-basket"></i> Venta #@paddy($this->factura_t->model_id)</a>
        @endif
      ...
      <div class="row layout-top-spacing">
        <div class="col-12">
          @if (!$this->loading)
            ...
            {{-- <a href="{{route('prueba')}}" class="btn btn-large btn-block btn-success"><i class="fas fa-file-alt"></i> GENERAR FACTURA</a>     --}}
          @else
            <center>
              <h4>Timbrando Factura....</h4>
            </center>
          @endif
